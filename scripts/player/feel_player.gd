...
# small safe camera tuning only
@export var force_camera_current: bool = true
@export var camera_near: float = 0.03
@export var camera_far: float = 500.0
@export var camera_exposure_multiplier: float = 1.12
@export var camera_auto_exposure_enabled: bool = false
...
func _setup_camera_visuals() -> void:
	if cam == null:
		return

	# make sure this is the actual gameplay camera
	if force_camera_current:
		cam.make_current()

	# better FPS clipping defaults
	cam.near = camera_near
	cam.far = camera_far

	# only touch camera attributes if needed
	var practical := cam.attributes as CameraAttributesPractical
	if practical == null:
		practical = CameraAttributesPractical.new()
		cam.attributes = practical

	practical.auto_exposure_enabled = camera_auto_exposure_enabled
	practical.exposure_multiplier = camera_exposure_multiplier
